    ...
    try {
      ...
      if (location) {
        ...
        if (d <= config.radio_maximo) {
          ...
          if (address) {
            setMensajeFlash(`Ubicacion valida: ${address.split(',')[0]}`);
          } else {
            setMensajeFlash('Ubicacion valida');
          }
        } else {
        ...
        <div className="w-full bg-[#111111] p-8 rounded-[35px] border border-white/5 shadow-2xl flex flex-col items-center">
          {!ubicacionOk ? (
            <div className="w-full py-10 bg-rose-500/10 rounded-[30px] border border-rose-500/20 text-center">
              <div className="w-12 h-12 rounded-full bg-rose-500/20 border border-rose-500/30 flex items-center justify-center mb-3">
