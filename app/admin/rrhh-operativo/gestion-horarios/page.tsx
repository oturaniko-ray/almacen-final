  ...
  const Memebrete = () => (
    <div className="w-full max-w-sm bg-[#1a1a1a] px-5 py-3 rounded-[18px] border border-white/5 mb-3 text-center shadow-xl">
      <h1 className="text-base font-black italic uppercase tracking-tighter leading-none mb-1">
        ...
        <span className="text-blue-400">HORARIOS</span>
      </h1>
      <div className="flex items-center justify-center gap-2 pt-1 border-t border-white/10 mt-1">
        ...
        <span className="text-xs text-blue-400 normal-case">{user.rol || 'Administrador'}</span>
      </div>
    </div>
  );
    ...
    <main className="min-h-screen bg-black flex flex-col items-center justify-start pt-10 px-4 pb-4 font-sans">
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full space-y-4">
          ...
          <BotonOpcion
            texto="DEFINIR TURNOS"
            descripcion="CREACIÓN Y CONFIGURACIÓN DE TURNOS. HORARIOS Y CAPACIDAD"
            onClick={() => router.push('/admin/turnos')}
            color="bg-indigo-600"
          />
          
          <BotonOpcion
            texto="PROGRAMACIÓN DE TURNOS"
            descripcion="ASIGNACIÓN DE TURNOS A EMPLEADOS. VISTA DE CALENDARIO SEMANAL"
            onClick={() => router.push('/admin/programacion')}
            color="bg-blue-600"
          />

        {/* BOTÓN 2: INTERCAMBIOS DE TURNOS */}
          <BotonOpcion
            texto="INTERCAMBIOS"
            descripcion="GESTIÓN DE SOLICITUDES DE INTERCAMBIO DE TURNOS"
            onClick={() => router.push('/admin/intercambios')}
            color="bg-purple-600"
          />
