...
export default function RRHHCategoriasPage() {
  ...
  const Memebrete = () => (
    <div className="w-full max-w-sm bg-[#1a1a1a] px-5 py-3 rounded-[18px] border border-white/5 mb-3 text-center shadow-xl">
      <h1 className="text-base font-black italic uppercase tracking-tighter leading-none mb-1">
        <span className="text-white">RRHH </span>
        <span className="text-teal-500">OPERATIVO</span>
      </h1>
      <div className="flex items-center justify-center gap-2 pt-1 border-t border-white/10 mt-1">
        ...
        <span className="text-xs text-teal-500 normal-case">{user.rol || 'Administrador'}</span>
  ...
  const Footer = () => (
    <div className="w-full max-w-sm mt-8 pt-4 border-t border-white/5 text-center">
      ...
      >
        <span className="text-lg">←</span> VOLVER A GESTIÓN ADMINISTRATIVA
      </button>
    </div>
  );
    ...
    <main className="min-h-screen bg-black flex flex-col items-center justify-start pt-10 px-4 pb-4 font-sans">
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full space-y-4">
          {/* CATEGORÍA 1: GESTIÓN Y PLANIFICACIÓN (ACTIVO) */}
          <BotonOpcion 
            texto="GESTIÓN Y PLANIFICACIÓN" 
            descripcion="horarios, Ausencias y reportes" 
            onClick={() => router.push('/admin/rrhh-operativo/planificacion')} 
            color="bg-blue-600" 
          />

          {/* CATEGORÍAS FUTURAS (DESHABILITADAS) */}
          <BotonOpcion 
            texto="NÓMINA" 
            descripcion="PRÓXIMAMENTE: Calculo de salarios y liquidaciones" 
            onClick={() => {}} 
            color="bg-gray-700" 
            disabled={true}
          />
          ...
          <BotonOpcion 
            ...
            descripcion="PRÓXIMAMENTE: Metricas y objetivos" 
            onClick={() => {}} 
            color="bg-gray-700" 
            disabled={true}
          />
          ...
          <BotonOpcion 
            ...
            descripcion="PRÓXIMAMENTE: Cursos y certificaciones" 
            onClick={() => {}} 
            color="bg-gray-700" 
            disabled={true}
          />
