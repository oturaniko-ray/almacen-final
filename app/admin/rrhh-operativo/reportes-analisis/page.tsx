...
export default function ReportesAnalisisPage() {
  ...
  const Memebrete = () => (
    <div className="w-full max-w-sm bg-[#1a1a1a] px-5 py-3 rounded-[18px] border border-white/5 mb-3 text-center shadow-xl">
      <h1 className="text-base font-black italic uppercase tracking-tighter leading-none mb-1">
        ...
        <span className="text-purple-400">ANÁLISIS</span>
      </h1>
      <div className="flex items-center justify-center gap-2 pt-1 border-t border-white/10 mt-1">
        ...
        <span className="text-xs text-purple-400 normal-case">{user.rol || 'Administrador'}</span>
      </div>
    </div>
  );
    ...
    <main className="min-h-screen bg-black flex flex-col items-center justify-start pt-10 px-4 pb-4 font-sans">
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full space-y-4">
          ...
          <BotonOpcion
            texto="TIMESHEET SEMANAL"
            descripcion="HORAS TRABAJADAS POR EMPLEADO. EFICIENCIA Y PRODUCTIVIDAD"
            onClick={() => router.push('/admin/reportes/timesheet')}
            color="bg-purple-600"
          />
          
          <BotonOpcion
            texto="COMPARATIVA TURNOS VS REALES"
            descripcion="ANÁLISIS DE ASISTENCIA. TURNOS PLANIFICADOS VS MARCADOS"
            onClick={() => router.push('/admin/reportes/comparativa')}
            color="bg-orange-600"
          />
