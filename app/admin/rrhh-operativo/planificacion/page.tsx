...
export default function GestionYPlanificacionPage() {
  ...
  const volverARRHH = () => {
    router.push('/admin/rrhh-operativo');
  };
  ...
  const Memebrete = () => (
    <div className="w-full max-w-sm bg-[#1a1a1a] px-5 py-3 rounded-[18px] border border-white/5 mb-3 text-center shadow-xl">
      <h1 className="text-base font-black italic uppercase tracking-tighter leading-none mb-1">
        <span className="text-white">GESTIÓN Y </span>
        <span className="text-blue-500">PLANIFICACIÓN</span>
      </h1>
  ...
  const Footer = () => (
    <div className="w-full max-w-sm mt-8 pt-4 border-t border-white/5 text-center">
      ...
      >
        <span className="text-lg">←</span> VOLVER A RRHH OPERATIVO
      </button>
    </div>
  );
    ...
    <main className="min-h-screen bg-black flex flex-col items-center justify-start pt-10 px-4 pb-4 font-sans">
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full space-y-4">
          ...
          {/* BOTÓN 1: GESTIÓN DE HORARIOS */}
          <BotonOpcion
            texto="GESTIÓN DE HORARIOS"
            descripcion="DEFINICIÓN Y ASIGNACIÓN DE TURNOS"
            onClick={() => router.push('/admin/rrhh-operativo/gestion-horarios')}
            color="bg-blue-600"
          />

          {/* BOTÓN 2: PLANIFICACIÓN DE PERSONAL */}
          <BotonOpcion
            texto="PLANIFICACIÓN DE PERSONAL"
            descripcion="VACACIONES, PERMISOS Y AUSENCIAS"
            onClick={() => router.push('/admin/rrhh-operativo/planificacion-personal')}
            color="bg-green-600"
          />

          {/* BOTÓN 3: REPORTES Y ANÁLISIS */}
          <BotonOpcion
            texto="REPORTES Y ANÁLISIS"
            descripcion="TIMESHEET Y COMPARATIVA DE ASISTENCIA"
            onClick={() => router.push('/admin/rrhh-operativo/reportes-analisis')}
            color="bg-purple-600"
          />
