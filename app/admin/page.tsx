        ...
        {vista === 'empleados' ? (
            ...
            <div className="bg-[#0f172a] rounded-[35px] border border-white/5 overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-white/5 uppercase text-slate-500 font-black">
                  <tr><th className="p-4">Status</th><th className="p-4">Empleado / Correo</th><th className="p-4">ID / PIN</th><th className="p-4">Rol</th><th className="p-4 text-right">Acciones</th></tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {empleados.map(emp => (
                    <tr key={emp.id} className="hover:bg-white/[0.02]">
                      <td className="p-4"><div className={`w-3 h-3 rounded-full ${emp.en_almacen ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500'}`}></div></td>
                      <td className="p-4"><p className="font-bold">{emp.nombre}</p><p className="text-[10px] text-slate-500">{emp.email}</p></td>
                      <td className="p-4"><p className="font-mono text-blue-400">{emp.documento_id}</p><p className="font-mono text-slate-500">PIN: {emp.pin_seguridad}</p></td>
                      ...
                      <td className="p-4 text-right flex justify-end gap-2">
                        ...
                        <button onClick={async () => { await supabase.from('empleados').update({ activo: !emp.activo }).eq('id', emp.id); fetchEmpleados(); }} className={`px-3 py-1 rounded-full font-black text-[9px] ${emp.activo ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>{emp.activo ? 'DESACTIVAR' : 'ACTIVAR'}</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#0f172a] rounded-[35px] border border-white/5 overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-white/5 uppercase text-slate-500 font-black">
                  <tr><th className="p-4">Empleado</th><th className="p-4">Acción</th><th className="p-4">Detalles / Autorización</th><th className="p-4">Fecha y Hora</th></tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {movimientos.map(mov => (
                    <tr key={mov.id}>
