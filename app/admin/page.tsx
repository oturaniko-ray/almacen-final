    ...
    <main className="min-h-screen bg-[#050a14] text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-black text-blue-500 uppercase tracking-widest">ADMINISTRACIÓN</h1>
          <div className="flex gap-4">
            {/* BOTÓN ACTUALIZAR */}
            <button onClick={cargarDatos} className="px-6 py-2 bg-slate-800 hover:bg-blue-600 rounded-xl font-bold transition-all shadow-lg border border-white/10">
              🔄 Actualizar
            </button>
        ...
        <div className="bg-[#0f172a] rounded-[30px] border border-white/5 overflow-hidden shadow-2xl">
          {/* CONTENEDOR CON SCROLL PARA ~6 FILAS Y CABECERA PEGAJOSA */}
          <div className="max-h-[500px] overflow-y-auto">
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-[#1e293b] z-20 shadow-md">
                <tr className="text-[10px] uppercase tracking-widest text-slate-400">
                  <th className="p-6">Nombre / Empleado</th>
                  <th className="p-6">{view === 'movimientos' ? 'Tipo' : 'Email'}</th>
                  <th className="p-6">{view === 'movimientos' ? 'Fecha' : 'Rol'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {(view === 'movimientos' ? logs : empleados).map((item) => (
                  <tr key={item.id} className="hover:bg-white/5 transition-all">
                    <td className="p-6 font-bold">{item.nombre || item.nombre_empleado}</td>
                    <td className="p-6">
                      {view === 'movimientos' ? (
