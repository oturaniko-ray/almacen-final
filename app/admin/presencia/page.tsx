...
export default function ReportesPage() {
  ...
  useEffect(() => {
    ...
    fetchJornadas();
    const ch = supabase.channel('jornadas').on('postgres_changes', { event: '*', schema: 'public', table: 'jornadas' }, () => fetchJornadas()).subscribe();
    ...
    <main className="min-h-screen bg-[#050a14] p-8 text-white font-sans">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-black uppercase italic text-blue-500">Reporte <span className="text-white">General</span></h1>
          <div className="flex gap-4">
            <button onClick={exportarExcel} className="bg-emerald-600 px-6 py-2 rounded-xl text-[10px] font-black uppercase">Excel</button>
        ...
        <div className="flex flex-wrap gap-4 mb-8 bg-[#0f172a] p-6 rounded-[30px] border border-white/5">
          ...
          <input type="date" className="bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-[11px] font-bold uppercase outline-none focus:border-blue-500" value={desde} onChange={e => setDesde(e.target.value)} />
          <input type="date" className="bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-[11px] font-bold uppercase outline-none focus:border-blue-500" value={hasta} onChange={e => setHasta(e.target.value)} />
        </div>
            ...
            <tbody className="divide-y divide-white/5">
              {jornadasFiltradas.map((j) => (
                <tr key={j.id} className="hover:bg-white/[0.01] transition-colors">
                  ...
                  <td className="p-6 text-lg font-black text-yellow-400 italic">{j.estado === 'activo' ? 'EN CURSO' : j.horas_trabajadas}</td>
                  <td className="p-6">
