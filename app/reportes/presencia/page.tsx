              ...
              {jornadasFiltradas.map((j) => {
                ...
                return (
                  <React.Fragment key={j.id}>
                    {mostrarSeparador && (
                      <tr className="bg-white/5"><td colSpan={5} className="px-6 py-2 text-[9px] font-black text-blue-400 uppercase tracking-[0.3em]">📅 {fechaFila}</td></tr>
                    )}
                    <tr className="hover:bg-white/[0.01] border-b border-white/5">
                      ...
                      <td className="p-6 font-black text-yellow-400 italic tracking-tighter">
                        <span className="text-[12px] opacity-90">{j.estado === 'activo' ? 'EN PROCESO...' : j.horas_trabajadas || '00:00:00'}</span>
                      </td>
