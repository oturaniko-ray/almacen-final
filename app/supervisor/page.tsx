...
const TIEMPO_MAX_TOKEN_MS = 120000; // 2 minutos en milisegundos
...
export default function SupervisorPage() {
  ...
  const registrar = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        // 1. Validar ubicación del supervisor
        const dSup = calcularDistancia(pos.coords.latitude, pos.coords.longitude, ALMACEN_LAT, ALMACEN_LON);
        if (dSup > RADIO_MAXIMO_METROS) throw new Error(`Fuera de rango (${Math.round(dSup)}m)`);

        // 2. Decodificar Token y Validar Tiempo
        let documentoId = qrData;
        try {
          const decoded = atob(qrData).split('|');
          if (decoded.length === 2) {
            documentoId = decoded[0];
            const timestampToken = parseInt(decoded[1]);
            const ahora = Date.now();
            
            if (ahora - timestampToken > TIEMPO_MAX_TOKEN_MS) {
              throw new Error("TOKEN EXPIRADO: Pida al empleado refrescar su código QR.");
            }
          }
        } catch (e) {
          // Si no es Base64, asumimos que es entrada manual o USB directo
          if (modo === 'camara') throw new Error("QR no válido o corrupto.");
        }

        // 3. Validar Empleado
        const { data: emp, error: errEmp } = await supabase.from('empleados').select('*').eq('documento_id', documentoId).maybeSingle();
        if (errEmp || !emp) throw new Error("Empleado no registrado.");
        if (!emp.activo) throw new Error("Empleado inactivo.");

        // 4. Validar Supervisor
        ...
        if (errInsert) throw new Error("Fallo al guardar en historial.");

        // 6. Actualizar Estatus
        await supabase.from('empleados').update({ en_almacen: (direccion === 'entrada') }).eq('id', emp.id);

        alert(`REGISTRO EXITOSO: ${emp.nombre} - ${direccion?.toUpperCase()}`);
        handleVolver();
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#050a14] p-6 rounded-[30px] border border-white/5">
              <input type="text" className="bg-transparent text-blue-400 font-mono font-bold text-center w-full outline-none uppercase text-lg" 
                placeholder={qrData ? "ID: " + qrData.slice(0,8) : "Esperando Lectura..."} value={qrData} onChange={(e)=>setQrData(e.target.value)} autoFocus={modo !== 'camara'} />
            </div>
