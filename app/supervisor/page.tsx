...
export default function SupervisorPage() {
  ...
  const handleVolver = async () => {
    if (qrData || documentoManual) {
      ...
      setDireccion(null); await stopScanner();
    } else if (modo !== 'menu') {
      setModo('menu'); await stopScanner();
    } else {
      router.push('/');
    }
  };
  ...
  const registrar = async () => {
    ...
    if (error || !emp || !emp.activo) {
      playSound('error'); alert(!emp?.activo ? "❌ EMPLEADO DESACTIVADO" : "❌ PIN/ID INCORRECTO"); setPin(''); return;
    }
    ...
    if (!regError) {
      ...
      playSound('success'); setQrData(''); setPin(''); setDocumentoManual(''); setModo('menu'); setDireccion(null);
  ...
  return (
    <main className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center p-6 text-white font-sans">
      <style>{`
        @keyframes glow {
          0%, 100% { text-shadow: 0 0 5px #3b82f6, 0 0 10px #3b82f6; opacity: 1; }
          50% { text-shadow: 0 0 20px #3b82f6, 0 0 30px #3b82f6; opacity: 0.7; }
        }
      ...
      <div className="bg-[#0f172a] p-10 rounded-[45px] w-full max-w-lg border border-white/5 text-center shadow-2xl relative overflow-hidden">
        {animar && <div className="absolute inset-0 bg-blue-600/20 z-50 flex items-center justify-center backdrop-blur-sm animate-pulse"><span className="font-black text-2xl italic">PROCESANDO...</span></div>}
        ...
        {modo === 'menu' ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#050a14] p-8 rounded-[30px] border border-white/5 flex flex-col items-center justify-center">
              {modo === 'manual' ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  {/* PUNTO 2: ICONO ANIMADO Y EFECTO DESTELLANTE */}
                  {!qrData && (
                    <div className="w-12 h-12 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div>
                  )}
                  <p className={`text-blue-400 font-mono font-bold text-2xl uppercase tracking-tighter ${!qrData ? 'animate-glow' : ''}`}>
                    {qrData.split('|')[0] || "Esperando Lectura"}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              {/* PUNTO 2: REDUCCIÓN TAMAÑO TEXTO PIN */}
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">PIN DE SEGURIDAD</p>
              <input 
                ref={pinRef}
                type="password" 
                placeholder="****" 
                className="w-full py-6 bg-[#050a14] rounded-[30px] text-white text-center text-5xl font-black outline-none border-2 border-transparent focus:border-blue-500 transition-all" 
                value={pin} 
