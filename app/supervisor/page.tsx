...
export default function SupervisorPage() {
  ...
  // Estado para el emergente ámbar de 2 segundos
  ...
  // Función para manejar el fuera de rango solicitado
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      ...
      if (data) {
        ...
        // Conversión estricta de radio_maximo de texto a número
  ...
  useEffect(() => {
    if (config.lat !== 0 && gps.lat !== 0) {
      ...
      setGps(prev => {
        // Validación en tiempo de lectura: si sale del rango, vuelve atrás y borra buffer
  ...
  // Bloqueo de entrada a submenús si está fuera de rango
  ...
  const registrarAcceso = async () => {
    // Re-validación de rango antes de confirmar registro
    if (gps.dist > config.radio) {
      manejarFueraDeRango(gps.dist);
      return;
    }
    setAnimar(true);
  ...
  return (
    <main className="min-h-screen bg-black flex flex-col items-center justify-center p-4 relative font-sans overflow-hidden">
      ...
      {/* EMERGENTE ÁMBAR DE 2 SEGUNDOS SOLICITADO */}
      {alertaGps.visible && (
