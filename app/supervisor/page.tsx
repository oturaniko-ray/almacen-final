const registrarAcceso = async () => {
  ...
  // --- Validar PIN del autorizador según el modo ---
  let autorizador = null;
  let errorAutorizador = null;

  if (modo === 'manual') {
    // MODO MANUAL: Validar PIN del supervisor presente (sin importar quién está logueado)
    const result = await supabase
      .from('empleados')
      .select('nombre, rol, nivel_acceso')
      .eq('pin_seguridad', String(pinAutorizador))
      .gte('nivel_acceso', 3) // Solo niveles 3 o superiores
      .maybeSingle();
    ...
    if (errorAutorizador || !autorizador) {
      mostrarNotificacion('PIN DE SUPERVISOR INVÁLIDO (SE REQUIERE NIVEL ≥ 3)', 'error');
      setAnimar(false);
    ...
    // Verificar que el usuario logueado tenga nivel ≥ 3
  ...
  // Función para redireccionar según nivel de acceso
  const redireccionarSegunNivel = (nivel: number) => {
    setTimeout(() => {
      if (nivel >= 8) {
        router.push('/tecnico');
      } else if (nivel >= 4) {
        ...
        // Niveles 1-2 se quedan en la misma página
        if (modo === 'manual') {
          setPasoManual(1);
          setQrData('');
          setQrInfo(null);
  ...
  try {
    ...
    redireccionarSegunNivel(autorizador.nivel_acceso);
