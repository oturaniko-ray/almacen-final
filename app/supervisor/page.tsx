...
export default function SupervisorPage() {
  ...
  // --- LÓGICA ESCÁNER USB (CORREGIDA) ---
  useEffect(() => {
    if (modo !== 'usb' || !direccion || lecturaLista) return;

    let buffer = "";
    const handleKeyDown = (e: KeyboardEvent) => {
      // Evitamos capturar si el foco está en un input (como el PIN)
      ...
      if (e.key === 'Enter') {
        if (buffer.length > 2) {
          ...
          // Enfocamos el PIN del supervisor automáticamente tras la lectura
          setTimeout(() => pinRef.current?.focus(), 250);
        }
        buffer = "";
      } else {
        // Los escáneres USB suelen enviar caracteres muy rápido
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        ...
        if (direccion === 'entrada') {
          ...
          if (!jActiva) throw new Error("No hay entrada activa registrada");
          ...
          await supabase.from('jornadas').update({ 
            ...
            estado: 'finalizado', 
            editado_por: `Aut: ${aut.nombre}` 
          }).eq('id', jActiva.id);
          await supabase.from('empleados').update({ en_almacen: false }).eq('id', emp.id);
        }

        alert(`✅ Éxito: ${emp.nombre}`);
        setQrData(''); setPinAutorizador(''); setPinEmpleadoManual(''); setLecturaLista(false); setAnimar(false);
    ...
    <main className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center p-6 text-white font-sans relative overflow-hidden">
      ...
      <div className="bg-[#0f172a] p-10 rounded-[45px] w-full max-w-lg border border-white/5 shadow-2xl relative z-10">
        <div className="mb-6 text-center">
          <h2 className="text-2xl font-black uppercase italic text-blue-500 tracking-tighter italic">Panel de Supervisión</h2>
          {user && (
            <div className="mt-2">
              <p className="text-[11px] font-bold text-slate-300 uppercase tracking-widest">{user.nombre} <span className="text-blue-400">[{user.rol}]</span></p>
              <p className="text-[9px] font-black text-slate-500 uppercase mt-1 italic">🛰️ GPS: {gpsReal.lat.toFixed(6)} | {distanciaAlmacen ?? '--'}m</p>
            </div>
          )}
        </div>
        ...
        {modo === 'menu' ? (
          <div className="grid gap-4">
            <button onClick={() => setModo('usb')} className="p-8 bg-[#1e293b] rounded-[30px] font-black text-lg border border-white/5 uppercase hover:border-blue-500 transition-all">🔌 Escáner USB</button>
            <button onClick={() => setModo('camara')} className="p-8 bg-[#1e293b] rounded-[30px] font-black text-lg border border-white/5 uppercase hover:border-emerald-500 transition-all">📱 Cámara Móvil</button>
            <button onClick={() => setModo('manual')} className="p-8 bg-[#1e293b] rounded-[30px] font-black text-lg border border-white/5 uppercase hover:border-slate-500 transition-all">🖋️ Manual</button>
            <button onClick={() => router.push('/')} className="mt-6 text-slate-500 font-bold uppercase text-[11px] text-center tracking-widest">← Volver</button>
          </div>
        ) : !direccion ? (
          <div className="flex flex-col gap-6">
            <button onClick={() => setDireccion('entrada')} className="w-full py-12 bg-emerald-600 rounded-[35px] font-black text-4xl shadow-xl hover:scale-105 transition-transform uppercase">ENTRADA</button>
            <button onClick={() => setDireccion('salida')} className="w-full py-12 bg-red-600 rounded-[35px] font-black text-4xl shadow-xl hover:scale-105 transition-transform uppercase">SALIDA</button>
            <button onClick={volverAtras} className="mt-4 text-slate-500 font-bold text-[10px] uppercase text-center tracking-widest">← Cambiar Modo</button>
          </div>
        ) : (
          <div className="space-y-6">
            <div className={`bg-[#050a14] p-6 rounded-[30px] border transition-all ${lecturaLista ? 'border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.2)]' : 'border-white/5'} relative overflow-hidden h-64 flex flex-col items-center justify-center shadow-inner`}>
              {!lecturaLista ? (
                <>
                  ...
                  {modo === 'usb' && (
                    <div className="text-center space-y-2">
                      <p className="text-[12px] font-black text-blue-500 uppercase animate-pulse">Escanee el código QR</p>
                      <p className="text-[9px] font-bold text-slate-600 uppercase tracking-tighter italic">Esperando entrada de periférico USB...</p>
                    </div>
                  )}
                </>
              ) : (
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-widest">Proceda con el PIN de autorización</p>
                </div>
              )}
            </div>
            ...
            {(lecturaLista || modo === 'manual') && (
              <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
                {modo === 'manual' && (
                  <input type="text" className="w-full py-4 bg-[#050a14] rounded-[20px] text-center text-xl font-bold border border-white/10 text-white outline-none focus:border-blue-500 transition-colors" placeholder="ID Empleado" value={qrData} onChange={(e) => setQrData(e.target.value)} />
                )}
            ...
            <button onClick={volverAtras} className="w-full text-center text-slate-600 font-bold uppercase text-[9px] tracking-[0.3em] hover:text-red-500 transition-colors">✕ CANCELAR OPERACIÓN</button>
          </div>
        )}
      </div>
    </main>
