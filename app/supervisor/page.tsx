  ...
  const registrarAcceso = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        ...
        if (dEntera > config.radio_maximo) throw new Error(`Fuera de rango (${dEntera}m). Máx: ${config.radio_maximo}m`);
        ...
        {modo === 'menu' ? (
          ...
          <div className="space-y-6">
            ...
            {(lecturaLista || modo === 'manual') && (
              <input ref={pinRef} type="password" placeholder="PIN Supervisor" className="w-full py-5 bg-[#050a14] rounded-[25px] text-center text-3xl font-black border-2 border-blue-500/20 outline-none" value={pinAutorizador} onChange={(e) => setPinAutorizador(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') registrarAcceso(); }} />
            )}
